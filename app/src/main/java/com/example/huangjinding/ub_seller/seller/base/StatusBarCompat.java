...
public class StatusBarCompat {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (style == Style.NORMAL) {
                ...
                if (drawerLayout instanceof DrawerLayout) {
                    ...
                    int drawerContentViewIndex = drawerLayout
                            .indexOfChild(drawerContentView);
                    ...
                    HackFrameLayout hackFrameLayout = new HackFrameLayout(
                            activity);
                    hackFrameLayout.addView(drawerLayout,
                            new ViewGroup.LayoutParams(
                                    ViewGroup.LayoutParams.MATCH_PARENT,
                                    ViewGroup.LayoutParams.MATCH_PARENT));
                    rootView.addView(hackFrameLayout,
                            new ViewGroup.LayoutParams(
                                    ViewGroup.LayoutParams.MATCH_PARENT,
                                    ViewGroup.LayoutParams.MATCH_PARENT));
                }
    ...
    private static int getStatusBarHeight(Activity activity) {
        ...
        return resourceId > 0 ? activity.getResources().getDimensionPixelSize(
                resourceId) : 0;
    }
